...
var update = async function(newData, eNo){
    ...
    await query('UPDATE exhibition SET eName=?, eFile2D=?, introdution=?,startTime=?,endTime=?,eImage=?,eType=?,eDelete=?,style=? WHERE eNo = ?', [newData.eName, newData.eFile2D, newData.introdution,newData.startTime, newData.endTime, newData.eImage, newData.eType,newData.eDelete,newData.style, eNo])
        .then((data) => {

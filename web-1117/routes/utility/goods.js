...
var update = async function(newData, gNo){
    ...
    await query('UPDATE goods SET gName=?, introdution=?, gAmount=?,price=?,gImage2D=?,gDelete=? WHERE gNo = ?', [newData.gName, newData.introdution,newData.gAmount,newData.price, newData.gImage2D,newData.gDelete, gNo])
        .then((data) => {
